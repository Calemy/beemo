...
export default async function(fastify, opts){
    ...
    fastify.post('/channels/:id/messages', async (req, reply) => {
        ...
        let lastMessage = await database.db("lazer").collection("messages").find({}).sort({ id: -1 }).limit(1).toArray()

        if(lastMessage.length < 1) lastMessage = [{ id: 0 }]
        
        await database.db("lazer").collection("messages").insertOne({
        ...
        let Channel = await database.db("lazer").collection("channels").findOne({ id: parseInt(req.params.id) })
        ...
        await database.db("lazer").collection("channels").findOneAndUpdate({ id: parseInt(req.params.id) }, { $set: Channel })
    ...
    fastify.get('/channels', async (req, res) => {
        const session = sessions.get(req.headers.authorization.split(" ")[1])
        if(!session) return { error: "Unauthorized User" }
        const channels = await database.db("lazer").collection("channels").find({}).toArray()
    ...
    fastify.get('/updates', async(req, reply) => {
        ...
        const messages = await database.db("lazer").collection("messages").find({ id: { $gt : parseInt(req.query.since) } }).sort({ id: -1 }).toArray()
